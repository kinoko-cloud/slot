...
| モード | 時間帯 | データソース | 用途 |
|--------|--------|-------------|------|
| **確定データ** | 閉店後〜開店前 (22:50-9:59) | 蓄積DB（前日までの確定実績） | 翌日の予測・おすすめ文 |
| **リアルタイム** | 営業中 (10:00-22:49) | 蓄積DB + availability.json | 当日の空き台・稼働状況反映 |

- 確定データモードでは**変動しない**。同じデータから同じ結果が出る（冪等）
- リアルタイムモードでは空き状況・稼働状況が反映されるが、**基本スコアは確定データと同一**
