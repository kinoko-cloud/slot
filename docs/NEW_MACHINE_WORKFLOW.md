...
### 機種名バリデーション（verify_keywords）

**必須設定。** スクレイピング時に台番号が別機種に入れ替わっていないか検出するためのキーワードリスト。

| パラメータ | 説明 | 例 |
|-----------|------|------|
| `verify_keywords` | ページ上の機種名に含まれるべきキーワード（リスト） | `['ブラックジャック']` |

⚠️ **同シリーズ機種との区別に注意：**
- 「北斗」だけだと北斗無双・北斗SE等と混同する → `['北斗', '転生']` のように複数キーワードで絞る
- 新シリーズが出たら（例: 転生3）、既存の `verify_keywords` も見直す
...
'new_machine_key': {
    ...
    'display_name': '新機種',
    'icon': '🎰',
    'verify_keywords': ['新機種'],  # ← 同シリーズ他機種と区別できるキーワード
...
- [ ] `config/rankings.py` — `verify_keywords` 設定（同シリーズ他機種と区別できるか確認）
