...
from analysis.ceiling_analyzer import (
    analyze_hit_pattern,
    estimate_true_setting,
    detect_reset,
    is_target_for_realtime,
    CEILING_PARAMS,
)
...
# 機種別閾値（攻略情報に基づく正確な値）
# SBJ: 設定6=1/165, 設定1=1/215（BB+RB合算）
# 北斗2: 設定6=1/273, 設定1=1/366
MACHINE_THRESHOLDS = {
    'sbj': {
        'setting6_at_prob': 165,   # 設定6相当
        'high_at_prob': 180,       # 高設定域（設定4-5）
        'mid_at_prob': 200,        # 中間域（設定2-3）
        'low_at_prob': 215,        # 設定1相当
        'very_low_at_prob': 250,   # 設定1以下（天井頼り）
    },
    'hokuto2': {
        'setting6_at_prob': 273,   # 設定6相当
        'high_at_prob': 300,       # 高設定域（設定4-5）
        'mid_at_prob': 340,        # 中間域（設定2-3）
        'low_at_prob': 366,        # 設定1相当
        'very_low_at_prob': 450,   # 設定1以下
    },
}
...
def analyze_today_data(unit_data: dict, current_hour: int = None, machine_key: str = 'sbj') -> dict:
    ...
    # === 天井・モード分析（新ロジック） ===
    # ceiling_analyzer を使用して天井到達率や真の設定を推定
    history = today_data.get('history', [])
    if history and result['art_count'] > 0:
        # 天井パターン分析
        hit_analysis = analyze_hit_pattern(history, machine_key)
        result['ceiling_analysis'] = hit_analysis
        
        ceiling_rate = hit_analysis.get('ceiling_rate', 0)
        ceiling_hits = hit_analysis.get('ceiling_hits', 0)
        is_upper_at_after = hit_analysis.get('is_upper_at_after', False)
        ...
        result['true_setting'] = setting_info
        
        # 天井到達率による評価
        if ceiling_rate <= 0.1:
            # 天井にほとんど行っていない = 高設定の可能性
            result['today_score_bonus'] += 15
            result['today_reasons'].append(
                f'🎯 天井到達率低い({ceiling_rate*100:.0f}%) → 高設定の可能性'
            )
        elif ceiling_rate >= 0.4:
            # 天井に頼りすぎ = 低設定の可能性
            result['today_score_bonus'] -= 20
            result['today_reasons'].append(
                f'⚠️ 天井到達率高い({ceiling_rate*100:.0f}%) → 低設定の可能性'
            )
        
        # 天井補正後の設定推定による評価
        adjusted_prob = setting_info.get('adjusted_prob', 0)
        estimated_setting = setting_info.get('estimated_setting', 0)
        
        if estimated_setting >= 5:
            result['today_score_bonus'] += 20
            result['today_reasons'].append(
                f'🎯 天井補正後: 推定設定{estimated_setting}（補正確率1/{adjusted_prob:.0f}）'
            )
        elif estimated_setting <= 2 and setting_info.get('confidence') in ['high', 'medium']:
            result['today_score_bonus'] -= 15
            result['today_reasons'].append(
                f'⚠️ 天井補正後: 推定設定{estimated_setting}（補正確率1/{adjusted_prob:.0f}）'
            )
        
        # 北斗2: 上位AT後（128あべし天国）の検出
        if machine_key == 'hokuto2' and is_upper_at_after:
            result['today_score_bonus'] += 40
            result['today_reasons'].append(
                f'🏆 上位AT後検出 → 128あべし天国！即座に狙い目！'
            )
            result['is_upper_at_after'] = True
