# 言語設定
- すべての応答は日本語で行うこと
- AskUserQuestionツールの質問文、選択肢のラベル・説明もすべて日本語で記述すること
- タスク管理（TaskCreate/TaskUpdate）のsubject、description、activeFormも日本語で記述すること
  - 例: subject「テストを実行する」、activeForm「テストを実行中...」
- コミットメッセージ、PRタイトル・説明も日本語で記述すること
- 技術用語やコード識別子は原文のままでよい
...
**⚠️ G数とあべしは比例しない（重要）:**
- あべしptは実機画面に表示される独自のカウンター
- レア役（チェリー・スイカ等）で大量加算されるため、G数とは無関係に増える
- **例: 100Gしか回してなくても画面に1500あべしと表示されることがある**
- データサイトからはG数しか取得できない（あべし数は取得不可）
- → G数ベースの天井判定は**参考値**であり、確定判定はできない
- `normal_ceiling=1100`はモードA（1536あべし）のG数参考上限値
