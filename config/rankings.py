...
MACHINES = {
    ...
    'hokuto_tensei2': {
        ...
        # 天井パラメータ
        # ※北斗転生2の天井は「あべし1536pt（モードA）」で、G数ではない
        #   レア役でもあべし加算されるため、G数ベースでは早めに到達する
        #   実データ: 最大ハマリ1146G、1000G超はわずか3件/665件
        #   → G数での実質天井は約1000〜1200G程度
        'normal_ceiling': 1100,       # 通常天井（G数換算。あべし1536≒実G数1100前後）
        'normal_ceiling_abeshi': 1536, # 液晶あべし表示上の天井
        'reset_ceiling': 600,         # リセット時天井（朝イチ天井、G数換算）
